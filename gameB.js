...
let cat = [[],[],[],[],[],[],[],[],[],[],[],[],[]], drone = [tryToLoad("drone[0]","grey"), tryToLoad("drone[1]","grey")];
...
let enemies = {x:1134,y:580,w:180,h:180,dx:-acceleration*3,dy:0,animation:0}, isOnEnemy = false;
...
function update() {
  if(!editMode){
    ...
    for(let i = 0; i<lvlData.collBox.x.length; i++){
      ...
      if(areColliding(myX + 5, myY + myH, myW - 10, 1, enemies.x+20, enemies.y+100, enemies.w-50, enemies.h)){
        myY -= myDY;
        myX += enemies.dx
        cameraX += enemies.dx
        myDY = 0;
      ...
      if (areColliding(myX, myY + 5, 1, myH - 10, enemies.x+20, enemies.y+100, enemies.w-50, enemies.h)) {
        myX -= myDX;
        cameraX -= myDX;
      ...
      if (areColliding(myX + myW, myY + 5, 1, myH - 10, enemies.x+20, enemies.y+100, enemies.w-50, enemies.h)) {
        myX -= myDX;
        cameraX -= myDX;
        enemies.dx = -enemies.dx;
        myDX = 0;
...
function draw() {
  drawImage(lvlBDoor,0-cameraX,0);
  drawImage(lvlBNDoor,2400-cameraX,1);
  ...
  if(!editMode){
    ...
    drawImage(drone[enemies.animation],enemies.x-cameraX,enemies.y,enemies.w,enemies.h)
    if(flipAni==0){
